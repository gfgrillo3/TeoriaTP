...
public class FirstFollow {
	...
	public static char[] getFirstVariable(String variable, Gramatica gramatica) {
		...
		for (Produccion produccion : gramatica.getProduccionesVariable(variable)) {
			...
			if ((97 <= asciiFirstCuerpoProduccion && asciiFirstCuerpoProduccion <= 122)
				...
				conjunto += (existeCharEnArray(conjunto.toCharArray(), primerCharProduccion) ? ""
						: primerCharProduccion);
			...
			for (int i = 0; produccion.getCuerpo().size() > i; i++) {
				...
				if (!esTerminal(produccion.getCuerpo().get(i))) {
					...
					if (produccion.getCuerpo().get(i).equals(variable) && i + 1 < produccion.getCuerpo().size()) {
						...
						char[] firstsVariable = getFirstVariable(produccion.getCuerpo().get(i + 1), gramatica);
						conjunto = addTerminales(conjunto, firstsVariable);
						...
						while (j + 1 < produccion.getCuerpo().size() && esAnulable) {
							...
							// si no existia agrego los char
							char[] firstsSimbolo = getFirstVariable(produccion.getCuerpo().get(j + 1), gramatica);
							conjunto = addTerminales(conjunto, firstsSimbolo);

							esAnulable = isAnulable(
									gramatica.getProduccionesVariable(produccion.getCuerpo().get(j + 1)));
							j++;
						}
					...
					// SI ERA TODO ANULABLE LO DE LA DERECHA O ERA EL ULTIMO, AGREGO LOS FOLLOWS DE
					// VARIABLE
					// SIEMPRE Y CUANDO EL LADO IZQUIERDO NO SEA ESA VARIABLE
	...
	private static String addTerminales(String conjunto, char[] firstsAgregar) {
		// si no existia agrego los char
		for(char c : firstsAgregar)
			if(!existeCharEnArray(conjunto.toCharArray(), c))
				conjunto += new String(c+"");
